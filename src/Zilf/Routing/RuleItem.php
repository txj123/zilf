...
class RuleItem extends Rule
{
     ...
     */
    public function __construct(Router $router, RuleGroup $parent, $name, $rule, $route, $method = '*', $option = [], $pattern = [])
    {
        $this->router = $router;
        $this->parent = $parent;
        $this->name = $name;
    ...
    protected function setRuleName($first = false)
    {
        if ($this->name) {
            ...
            Zilf::$container->getShare('ruleName')->set($name, $value, $first);
        }

        if (!$this->hasSetRule) {
            Zilf::$container->getShare('ruleName')->setRule($this->rule, $this);
     ...
     */
    public function checkRule($url, $match = null, $completeMatch = false)
    {
        if ($dispatch = $this->checkCrossDomain()) {
            // 允许跨域
            return $dispatch;
        }

        // 检查参数有效性
        if (!$this->checkOption($this->option)) {
            return false;
        }

        // 合并分组参数
        $option = $this->mergeGroupOptions();
        $url = $this->urlSuffixCheck($url, $option);
        ...
        if (false !== $match) {
            ...
            return $this->parseRule($this->rule, $this->route, $url, $option, $match);
        }
     ...
     * @return Dispatch|false
     */
    public function check($url, $completeMatch = false)
    {
        return $this->checkRule($url, null, $completeMatch);
    }
     ...
     * @return string
     */
    protected function urlSuffixCheck($url, $option = [])
    {
        // 是否区分 / 地址访问
    ...
    private function match($url, $option, $completeMatch)
    {
        ...
        $depr = $this->router->config('app.pathinfo_depr');

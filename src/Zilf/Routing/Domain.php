...
class Domain extends RuleGroup
{
     ...
     */
    public function __construct(Router $router, $name = '', $rule = null, $option = [], $pattern = [])
    {
        $this->router  = $router;
        $this->domain  = $name;
        $this->option  = $option;
    ...
    {
        // 检测别名路由
        $result = $this->checkRouteAlias($url);
        ...
        return parent::check($url, $completeMatch);
    }
     ...
     * @return Dispatch|false
     */
    private function checkUrlBind($url)
    {
        ...
        if (!empty($bind)) {
            ...
            if (isset($bindTo[$type])) {
                return $this->{$bindTo[$type]}($url, $bind);
            }
        }
    ...
    protected function bindToClass($url, $class)
    {
        ...
        return new CallbackDispatch($this, [$class, $action], $param);
    }

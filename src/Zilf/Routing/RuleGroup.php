...
use Zilf\Routing\Dispatch\Url as UrlDispatch;
...
class RuleGroup extends Rule
{
    ...
    public function check($url, $completeMatch = false)
    {
        ...
        // 检查分组有效性
        if (!$this->checkOption($this->option) || !$this->checkUrl($url)) {
            return false;
        }
        ...
        if ($this instanceof Resource) {
            ...
            if ($this->rule instanceof Response) {
                return new ResponseDispatch($this, $this->rule);
            }
        ...
        // 获取当前路由规则
        $method = strtolower(Zilf::$app->get('request')->getMethod());
        ...
        if (!empty($this->option['merge_rule_regex'])) {
            // 合并路由正则规则进行路由匹配检查
            $result = $this->checkMergeRuleRegex($rules, $url, $completeMatch);
        ...
        foreach ($rules as $key => $item) {
            $result = $item->check($url, $completeMatch);
        ...
        if ($this->auto) {
            // 自动解析URL地址
            $result = new UrlDispatch($this, $this->auto . '/' . $url, ['auto_search' => false]);
        } elseif ($this->miss && in_array($this->miss->getMethod(), ['*', $method])) {
            // 未匹配所有路由的路由规则处理
            $result = $this->miss->parseRule('', $this->miss->getRoute(), $url, $this->miss->mergeGroupOptions());
        } else {
            $result = false;
        }

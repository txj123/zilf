...
use Zilf\widgets\ContentDecorator;
use Zilf\widgets\FragmentCache;
...
class View extends Component implements DynamicContentAwareInterface
{
     ...
     * ```php
     * [
     *     'tpl' => ['class' => 'Zilf\smarty\ViewRenderer'],
     *     'twig' => ['class' => 'Zilf\twig\ViewRenderer'],
     * ]
     * ```
     *
    ...
    public function init()
    {
        ...
        if (is_array($this->theme)) {
            if (!isset($this->theme['class'])) {
                $this->theme['class'] = 'Zilf\Db\base\Theme';
            }
            $this->theme = Zilf::createObject($this->theme);
    ...
    protected function findViewFile($view, $context = null)
    {
        if (strncmp($view, '@', 1) === 0) {
            // e.g. "@app/views/main"
            $file = Zilf::getAlias($view);
        } elseif (strncmp($view, '//', 2) === 0) {
            // e.g. "//layouts/main"
            $file = Zilf::$app->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
        } elseif (strncmp($view, '/', 1) === 0) {
            // e.g. "/site/index"
            if (Zilf::$app->controller !== null) {
                $file = Zilf::$app->controller->module->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
            } else {
    ...
    {
        $viewFile = $requestedFile = Zilf::getAlias($viewFile);
        ...
        if ($this->beforeRender($viewFile, $params)) {
            ...
            if (isset($this->renderers[$ext])) {
                if (is_array($this->renderers[$ext]) || is_string($this->renderers[$ext])) {
                    $this->renderers[$ext] = Zilf::createObject($this->renderers[$ext]);
                }
    ...
    public function renderDynamic($statements)
    {
        if (!empty($this->cacheStack)) {
            ...
            $placeholder = "<![CDATA[Zilf-DYNAMIC-$n]]>";

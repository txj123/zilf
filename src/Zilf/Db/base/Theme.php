...
class Theme extends Component
{
    ...
    {
        $this->_baseUrl = $url === null ? null : rtrim(Zilf::getAlias($url), '/');
    }
    ...
    {
        $this->_basePath = Zilf::getAlias($path);
    }
    ...
    public function applyTo($path)
    {
        ...
        if (empty($pathMap)) {
            ...
            $pathMap = [Zilf::$app->getBasePath() => [$basePath]];
        }
        $path = FileHelper::normalizePath($path);
        foreach ($pathMap as $from => $tos) {
            $from = FileHelper::normalizePath(Zilf::getAlias($from)) . DIRECTORY_SEPARATOR;
            if (strpos($path, $from) === 0) {
                $n = strlen($from);
                foreach ((array) $tos as $to) {
                    $to = FileHelper::normalizePath(Zilf::getAlias($to)) . DIRECTORY_SEPARATOR;
